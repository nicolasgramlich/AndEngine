...
/**
 * An Entity is an object that can be drawn, generally something like a
 * {@link Sprite}, {@link Rectangle}, {@link Text} or a {@link Line}. An Entity
 * has a position/rotation/scale/color/etc. A {@link Scene} is an entity as
 * well. <br>
...
public class Entity implements IEntity {
	...
	/**
	 * Creates an Entity in the origin: coordinates (0, 0)
	 */
	public Entity() {
		this(0, 0);
	}
	...
	/**
	 * Creates a new entity given a pair of coordinates
	...
	@Override
	public void setRotationCenter(final float pRotationCenterX,
	...
	@Override
	public void setScaleCenter(final float pScaleCenterX,
	...
	public boolean isRotatedOrScaledOrSkewed() {
		return (this.mRotation != 0) || (this.mScaleX != 1)
				|| (this.mScaleY != 1) || (this.mSkewX != 0)
				|| (this.mSkewY != 0);
	}
	 ...
	 */
	@Override
	public void setColor(final float pRed, final float pGreen,
	...
	public <S extends IEntity> S queryFirstForSubclass(
		...
		for (int i = 0; i < childCount; i++) {
			...
			final S childQueryFirst = child
					.queryFirstForSubclass(pEntityMatcher);
	...
	@Override
	public <L extends List<IEntity>> L query(
			final IEntityMatcher pEntityMatcher, final L pResult) {
		final int childCount = this.getChildCount();
		for (int i = 0; i < childCount; i++) {
			final IEntity child = this.mChildren.get(i);
			if (pEntityMatcher.matches(child)) {
	...
	public boolean detachChild(final IEntity pEntity) {
		...
		return this.mChildren.remove(pEntity,
				Entity.PARAMETERCALLABLE_DETACHCHILD);
	}
	...
	public boolean detachChildren(final IEntityMatcher pEntityMatcher) {
		...
		return this.mChildren.removeAll(pEntityMatcher,
				Entity.PARAMETERCALLABLE_DETACHCHILD);
	}

	@Override
	public void callOnChildren(
			final IEntityParameterCallable pEntityParameterCallable) {
		if (this.mChildren == null) {
			return;
		}
	...
	@Override
	public boolean unregisterEntityModifier(
			final IEntityModifier pEntityModifier) {
		if (this.mEntityModifiers == null) {
			return false;
		}
	...
	public Transformation getLocalToSceneTransformation() {
		...
		final IEntity parent = this.mParent;
		if (parent != null) {
			localToSceneTransformation.postConcat(parent
	...
	public Transformation getSceneToLocalTransformation() {
		...
		final IEntity parent = this.mParent;
		if (parent != null) {
			sceneToLocalTransformation.preConcat(parent
	...
	public float[] convertLocalToSceneCoordinates(final float pX, final float pY) {
		return this.convertLocalToSceneCoordinates(pX, pY,
	 ...
	 */
	@Override
	public float[] convertLocalToSceneCoordinates(final float pX,
			final float pY, final float[] pReuse) {
		final Transformation localToSceneTransformation = this
				.getLocalToSceneTransformation();
	...
	public float[] convertLocalToSceneCoordinates(final float[] pCoordinates) {
		return this.convertLocalToSceneCoordinates(pCoordinates,
				Entity.VERTICES_LOCAL_TO_SCENE_TMP);
	}
	...
	public float[] convertSceneToLocalCoordinates(final float pX, final float pY) {
		return this.convertSceneToLocalCoordinates(pX, pY,
	 ...
	 */
	@Override
	public float[] convertSceneToLocalCoordinates(final float pX,
			final float pY, final float[] pReuse) {
		pReuse[Constants.VERTEX_INDEX_X] = pX;
		pReuse[Constants.VERTEX_INDEX_Y] = pY;
	...
	public float[] convertSceneToLocalCoordinates(final float[] pCoordinates) {
		return this.convertSceneToLocalCoordinates(pCoordinates,
				Entity.VERTICES_SCENE_TO_LOCAL_TMP);
	}
	...
	private void allocateEntityModifiers() {
		this.mEntityModifiers = new EntityModifierList(this,
				Entity.ENTITYMODIFIERS_CAPACITY_DEFAULT);
	}

	private void allocateChildren() {
		this.mChildren = new SmartList<IEntity>(
				Entity.CHILDREN_CAPACITY_DEFAULT);
	}

	private void allocateUpdateHandlers() {
		this.mUpdateHandlers = new UpdateHandlerList(
				Entity.UPDATEHANDLERS_CAPACITY_DEFAULT);
	}
	...
	protected void applyScale(final GLState pGLState) {
		...
		if ((scaleX != 1) || (scaleY != 1)) {
			...
			pGLState.translateModelViewGLMatrixf(-scaleCenterX, -scaleCenterY,
					0);
		}
	}
	...
	private void assertEntityHasNoParent(final IEntity pEntity)
			throws IllegalStateException {
		if (pEntity.hasParent()) {
			final String entityClassName = pEntity.getClass().getSimpleName();
			final String currentParentClassName = pEntity.getParent()
